...
{
    internal class Joshua : CorpsMember
    {
        public Joshua(string id)
        {
        ...
        private void InitialisePasssiveSkills(CorpsMember data)
        {
            ...
            Passives = data.Passives?.Select(p =>
            {
                ...
                {
                    "낙인" => () => skill.isActivated = true,
                    "죽음의 날개" => () => skill.isActivated = true,
                    "현트록스" => () => skill.isActivated = true,
                    _ => null
                };

                skill.Deactivate = p.Name switch
                {
                    "낙인" => () => skill.isActivated = false,
                    "죽음의 날개" => () => skill.isActivated = false,
                    "현트록스" => () => skill.isActivated = false,
                    _ => null
                };
        ...
        private void InitialiseActiveSkills(CorpsMember data)
        {
            ...
            Actives = data.Actives?.Select(a =>
            {
                ...
                {
                    "4군단식 질문" => () => skill.isOnCooldown = true,
                    "처형식" => () => skill.isOnCooldown = true,
                    _ => null
                };
                return skill;
            }).ToList() ?? [];
